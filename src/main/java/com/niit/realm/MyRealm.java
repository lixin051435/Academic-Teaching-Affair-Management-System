...
public class MyRealm extends AuthorizingRealm{
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
        ...
        Administrator administrator = administratorService.selectAdministratorById(id); //从数据库查询用户信息
        if (administrator != null) {
            roleNames.add("admin");
            permissionNames.add("admin");
        }
        Teacher teacher = teacherService.selectTeacherById(id); //从数据库查询用户信息
        if (teacher != null) {
            roleNames.add("teacher");
            permissionNames.add("teacher");
        }
        Student student = studentService.selectStudentById(id); //从数据库查询用户信息
        if (student != null) {
